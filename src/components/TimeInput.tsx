...
export const FULL_TIME_PATTERN = /^\d{1,2}:[0-5]\d$/
...
const TimeInput = ({ from, to, onFromChange, onToChange }: TimeInputProps) => {
    ...
    const handleFromBlur = (event: FocusEvent<HTMLInputElement>): void => {
        setFromInvalid(!FULL_TIME_PATTERN.test(event.target.value))
    }

    const handleToBlur = (event: FocusEvent<HTMLInputElement>): void => {
        setToInvalid(!FULL_TIME_PATTERN.test(event.target.value))
    }
        ...
        <div className="field">
            ...
            <div className="time-range">
                <div className="time-range-field">
                    ...
                    {fromInvalid && (
                        <p className="field-error">
                            {from === '' ? '"From" is required.' : 'Enter a valid time (MM:SS).'}
                        </p>
                    )}
                </div>
                ...
                <div className="time-range-field">
                    ...
                    {toInvalid && (
                        <p className="field-error">
                            {to === '' ? '"To" is required.' : 'Enter a valid time (MM:SS).'}
                        </p>
                    )}
                </div>
            </div>
            {rangeInvalid && <p className="time-range-error">"From" can't be the same or later than "To".</p>}
        </div>
    )
}
